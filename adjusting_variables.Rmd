...
Obtain the slope (coefficient) of x and the model intercept (where the slope intersects the y-axis), and visualize both variables:
...
ggplot(data, aes(x=x, y=y)) + 
  ...
  geom_point(alpha=.05) +
  geom_abline(intercept = intercept, slope = x_coef, colour="red", size=1) +
  geom_abline(intercept = intercept, slope = x_coef) +
