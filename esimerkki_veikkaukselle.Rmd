...
volatility_simulator <- function(minutes, ev, SD, players, topbottom=F, volatility="low") {
  ...
  for (i in 1:players) {
    ...
    #Low volatility: medium frequent wins
      ...
      test <- rnorm(minutes, mean=ev, sd=SD)
    }
...
```{r}
animation <- volatility_simulator(720, -0.5, 4, 50, T) + 
  geom_segment(aes(xend = 720, yend = value), linetype = 2, colour = 'grey') + 
  geom_point(size=2) +
  geom_text(aes(x = 710, label = key, color=profit), hjust = 0) +
...
```{r}
animation <- volatility_simulator(720, -0.5, 4, 50, T, "med") + 
  geom_segment(aes(xend = 720, yend = value), linetype = 2, colour = 'grey') + 
  geom_point(size=2) +
  geom_text(aes(x = 710, label = key, color=profit), hjust = 0) +
  geom_text(aes(x = ID, label = round(value, -1), color = profit), vjust=2) +
...
```{r}
animation <- volatility_simulator(720, -0.5, 4, 50, T, "high") + 
  geom_segment(aes(xend = 720, yend = value), linetype = 2, colour = 'grey') + 
  geom_point(size=2) +
  geom_text(aes(x = 710, label = key, color = profit), hjust = 0) +
  geom_text(aes(x = ID, label = round(value, -1), color = profit), vjust=2) +
...
```{r}
animation <- volatility_simulator(4320, -0.5, 4, 50, T, "high") + 
  geom_segment(aes(xend = 4320, yend = value), linetype = 2, colour = 'grey') + 
  geom_point(size=2) +
  geom_text(aes(x = 4300, label = key, color = profit), hjust = 0) +
  geom_text(aes(x = ID, label = round(value, -1), color = profit), vjust=2) +
  scale_color_manual(values=c("red", "darkgreen", "salmon", "lightblue")) +
  transition_reveal(ID) +
  coord_cartesian(clip = 'off')
