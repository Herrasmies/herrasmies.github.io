...
* Pelin palautusprosentti ≈ 90%
* Jokaisessa panostuskierroksessa on 1 euron panos
* Pelataan 5 panostuskierrosta minuutissa
* Käsien voittosumma (ja todennäköisyys):
  + Korkea volatiliteetti
    - Kuningasvärisuora: 500 (0.002%)
    - Värisuora: 55 (0.011%)
    - Neljä samaa: 45 (0.236%)
    - Täyskäsi: 13 (1.151%)
    - Väri: 7 (1.101%)
    - Suora: 6 (1.123%)
    - Kolme samaa: 3 (7.445%)
    - Kaksi paria: 2 (12.928%)
  + Matala volatiliteetti
    - Kuningasvärisuora: 100 (0.002%)
    - Värisuora: 50 (0.011%)
    - Neljä samaa: 30 (0.236%)
    - Täyskäsi: 15 (1.151%)
    - Väri: 8 (1.101%)
    - Suora: 7 (1.123%)
    - Kolme samaa: 3 (7.445%)
...
* Simulaatiot eivät välttämättä vastaa yksityiskohdiltaan esimerkiksi Veikkauksen tarjoamia videopokeripelejä
* Huom! Simulaatioissa oletetaan, että pelaajat pelaavat peliä optimistrategiaa hyödyntäen

<b>TL;DR:</b>

Alla esitetään videopokerisimulaatioita Mitä enemmän peliä pelaa, sitä enemmän rahaa häviää. Jotkut harvat ja onnekkaat voivat jäädä pelistä lyhyellä aikavälillä voitolle, erityisesti jos pelissä on korkea volatiliteetti, mutta riittävän pitkällä aikavälillä kaikki lopulta häviävät.
...
casino_simulator <- function(rounds, players, topbottom = F, volatility = "low") {
  ...
  if (volatility=="low") {
    ...
    payout <- c(500, 55, 45, 13, 7, 6, 3, 2, 0) - 1
    probability <- c(0.002, 0.011, 0.236, 1.151, 1.101, 1.123, 7.445, 12.928, 76.003)/100
    ev <- sum(payout*probability) #ev = -0.1076
  }
